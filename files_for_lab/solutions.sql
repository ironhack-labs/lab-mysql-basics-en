...
Query 3
To retrieve the 3 lowest amounts from the loan table, you can use the following SQL query:
...
SELECT account_id, SUM(amount) as COUNTER
FROM loan 
GROUP BY account_id 
ORDER BY COUNTER DESC
LIMIT 10;
...
Query 16
In the loan table, retrieve the number of loans issued for 
each day, before (excl) 930907, ordered by date in descending order.

SELECT `date`, count(`date`) AS LOANS
FROM loan
WHERE `date` < 930907
GROUP BY `date`
ORDER BY `date` DESC;
...
Query 17
In the loan table, for each day in December 1997, count the number of loans issued for each unique loan duration, ordered by date and duration, both in 
ascending order. You can ignore days without any loans in your output.

SELECT `date`, duration, count (`date`) AS loans
FROM loan 
